...
impl<'a, 'input> Attribute<'a, 'input> {
    ...
    /// Returns attribute's namespace prefix as it appeared in the source XML.
    ///
    /// Returns `None` if the attribute has no namespace prefix.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// ).unwrap();
    ///
    /// assert_eq!(doc.root_element().attributes().nth(0).unwrap().prefix(), None);
    /// assert_eq!(doc.root_element().attributes().nth(1).unwrap().prefix(), Some("n"));
    /// ```
    #[inline]
    pub fn prefix(&self) -> Option<&'input str> {
        self.data.name.namespace(self.doc).and_then(|ns| ns.name())
    }
...
impl<'a, 'input: 'a> Node<'a, 'input> {
    ...
    /// Returns the prefix of the element's tag name as it appeared in the source XML.
    ///
    /// Returns `None` if the element uses the default namespace (no prefix),
    /// if the element has no namespace, or if the current node is not an element.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// assert_eq!(child.tag_name_prefix(), Some("ns"));
    /// ```
    ///
    /// ```
    /// let doc = roxmltree::Document::parse(
    ...
    /// ).unwrap();
    /// assert_eq!(doc.root_element().tag_name_prefix(), None);
    /// ```
    #[inline]
    pub fn tag_name_prefix(&self) -> Option<&'input str> {
        match self.d.kind {
            NodeKind::Element { ref tag_name, .. } => {
                tag_name.namespace(self.doc).and_then(|ns| ns.name())
            }
            _ => None,
        }
    }
